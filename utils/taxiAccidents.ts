...
export function mergeAccidentsIntoDashboard(
  ...
  const repair = summary.totalCost;
  const ec = baseTotals.expenseByCategory;
  return {
    ...baseTotals,
    totalExpenses: baseTotals.totalExpenses + repair,
    netProfit:
      baseTotals.netProfit - repair + summary.totalInsuranceReceived,
    expenseByCategory:
      repair > 0
        ? {
            ...ec,
            other: ec.other + repair,
            grandTotal: ec.grandTotal + repair,
          }
        : ec,
  };
}
