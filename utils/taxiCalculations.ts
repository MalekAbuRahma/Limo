...
export function getPaymentStatus(remaining: number): 'مدفوع' | 'متأخر' {
  return remaining > 0 ? 'متأخر' : 'مدفوع';
}
...
export function computeEntry(
  ...
  const remaining = getRemaining(guarantee, entry.driverPaid);
  return {
    ...entry,
    expenseDetails,
    expenses,
    ...
    guarantee,
    remaining,
    status: getPaymentStatus(remaining),
...
export function computeDashboard(
  ...
  return {
    ...
    lateCount: computed.filter((e) => e.status === 'متأخر').length,
    paidCount: computed.filter((e) => e.status === 'مدفوع').length,
    expenseByCategory,
  };
}
