...
const lateOnly = filterEntries(entries, { query: '', status: 'متأخر', driver: 'all' });
assert(lateOnly.length > 0 && lateOnly.every((e) => e.status === 'متأخر'), 'status filter');
...
const searchToken = filterEntries(entries, { query: 'متأخر صيانة', status: 'all', driver: 'all' });
