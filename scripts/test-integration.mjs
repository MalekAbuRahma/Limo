/**
 * Integration tests: migration, backup, export headers, SQLite round-trip, API (optional)
...
import { initDb, getAppState, saveAppState, closeDb } from '../server/db.js';
...
const expenseKeys = [...VISIBLE_EXPENSE_KEYS];
const headers = [
  'الفترة',
...
const tmpDb = path.join(os.tmpdir(), `taxi-test-${Date.now()}.db`);
process.env.DB_PATH = tmpDb;
try {
  await initDb();
  const stateToSave = {
    settings: { ...DEFAULT_SETTINGS, monthlyGuarantee: 760, insuranceReceivedTotal: 100 },
    accidents: [
      migrateAccident({
        id: 'acc-db-1',
  ...
  assert(loaded.settings.monthlyGuarantee === 760, 'sqlite settings round-trip');
  assert(loaded.entries.length === 1, 'sqlite entries round-trip');
  assert(loaded.entries[0].driverName === 'أحمد', 'sqlite driver name');
  assert(loaded.entries[0].expenseDetails.oil === 30, 'sqlite expense breakdown');
  assert(
    sumExpenses(loaded.entries[0].expenseDetails) === loaded.entries[0].expenses,
    'sqlite expense sum'
  );
  assert(loaded.accidents.length === 1, 'sqlite accidents round-trip');
  assert(loaded.accidents[0].responsibleDriver === 'خالد', 'sqlite accident driver');
  assert(loaded.accidents[0].downtimeDays === 5, 'sqlite downtime_days column');
  assert(loaded.accidents[0].cost === 300, 'sqlite accident cost');
  assert(loaded.licenses.length === 1, 'sqlite licenses round-trip');
  assert(loaded.licenses[0].licenseDate === '2025-06-01', 'sqlite license date');
  assert(loaded.licenses[0].licenseYear === 2025, 'sqlite license year');
  assert(loaded.licenses[0].amountPaid === 85, 'sqlite license amount');
  assert(loaded.licenses[0].notes === 'تجديد', 'sqlite license notes');
  assert(loaded.oilChanges.length === 1, 'sqlite oil changes round-trip');
  assert(loaded.oilChanges[0].currentOdometer === 100000, 'sqlite odometer');
} finally {
...
try {
  ...
  if (health.ok) {
    ...
    assert(h.ok === true && h.storage === 'sqlite', 'API health');
