      ...
      <div className="bg-white rounded-lg w-full max-w-lg shadow-xl border border-slate-200 my-8 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor="handover-driver" className={labelClass}>{t.newDriverName}</label>
              <input id="handover-driver" type="text" required value={formData.driverName} onChange={e => setFormData({ ...formData, driverName: e.target.value })} className={inputClass} placeholder="Driver name" />
            </div>
            <div>
              <label htmlFor="handover-date" className={labelClass}>{t.startDate}</label>
              <input id="handover-date" type="date" required value={formData.startDate} onChange={e => setFormData({ ...formData, startDate: e.target.value })} className={inputClass} />
            </div>
          </div>
          <div>
            <label htmlFor="handover-notes" className={labelClass}>{t.notes}</label>
            <textarea id="handover-notes" value={formData.notes} onChange={e => setFormData({ ...formData, notes: e.target.value })} className={`${inputClass} min-h-[80px] resize-y`} placeholder="Optional notes" rows={3} />
          </div>
